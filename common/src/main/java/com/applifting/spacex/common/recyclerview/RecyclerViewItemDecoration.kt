package com.applifting.spacex.common.recyclerview
...
class RecyclerViewItemDecoration(
  ...
  @VisibleForTesting(otherwise = PRIVATE)
  internal val spacingParams: SpacingParams = SpacingParams(),
  ...
  private fun configSpacingForLinearLayoutManager(
    ...
    outRect.top = if (spacingParams.isTopSpacing) spacingPx else 0
    outRect.left = if (spacingParams.isLeftSpacing) spacingPx else 0
    if (layoutManager.canScrollHorizontally()) {
      outRect.right =
        if (spacingParams.isRightSpacing && position != itemCount - 1) spacingPx else 0

      outRect.bottom = if (spacingParams.isBottomSpacing) spacingPx else 0
    } else if (layoutManager.canScrollVertically()) {
      outRect.right = if (spacingParams.isRightSpacing) spacingPx else 0

      outRect.bottom =
        if (spacingParams.isBottomSpacing && position != itemCount - 1) spacingPx else 0
    }
  }
}
